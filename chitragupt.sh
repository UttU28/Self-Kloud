...
#
# Setup / use:
#   sudo ./chitragupt.sh mount        # fstab + folders (safe, no data touch)
#   sudo ./chitragupt.sh copy         # COPY selfHosted → chitragupt (originals kept)
#   sudo ./chitragupt.sh switch       # point .env at chitragupt (after verify copy)
#   sudo ./chitragupt.sh nextcloud    # wire Jellyfin into Nextcloud UI
...
# Pass "force" (or CHITRAGUPT_FORCE_MOUNT=1) to mount even when .env paths are local.
ensure_chitragupt_mounted() {
  local root="${CHITRAGUPT_ROOT}"
  local uuid="${CHITRAGUPT_UUID}"
  local force=0

  if [[ "${1:-}" == "force" || "${CHITRAGUPT_FORCE_MOUNT:-}" == "1" ]]; then
    force=1
  fi

  if [[ "$force" -eq 0 ]] && ! _uses_chitragupt_paths \
    "${NEXTCLOUD_DATA_PATH:-}" \
    "${NEXTCLOUD_DB_PATH:-}" \
    "${JELLYFIN_MEDIA_PATH:-}" \
    "${IMMICH_LIBRARY_PATH:-}" \
  ...
  if grep -qE "[[:space:]]${root//\//\\/}[[:space:]]" /etc/fstab 2>/dev/null; then
    ...
    _chitragupt_err "No fstab entry for ${root}. Run: sudo ~/Desktop/selfHosted/chitragupt.sh mount"
    return 1
  fi
...
_chitragupt_main() {
  ...
  do_copy() {
    ...
    rsync_if_needed "${DESKTOP}/jellyfin/media" "${CHITRAGUPT_ROOT}/jellyfin/media" "Jellyfin media"

    if [[ -d "${DESKTOP}/jellyfin/config" ]] && [[ -n "$(ls -A "${DESKTOP}/jellyfin/config" 2>/dev/null)" ]]; then
      rsync_if_needed "${DESKTOP}/jellyfin/config" "${CHITRAGUPT_ROOT}/jellyfin/config" "Jellyfin config"
    fi

    rsync_if_needed "${DESKTOP}/immich/library" "${CHITRAGUPT_ROOT}/immich/library" "Immich library"
    rsync_if_needed "${DESKTOP}/immich/postgres" "${CHITRAGUPT_ROOT}/immich/postgres" "Immich postgres"
    rsync_if_needed "${DESKTOP}/nextcloud/data" "${CHITRAGUPT_ROOT}/nextcloud/data" "Nextcloud app data"
    rsync_if_needed "${DESKTOP}/nextcloud/db" "${CHITRAGUPT_ROOT}/nextcloud/db" "Nextcloud MariaDB"
  ...
  do_switch() {
    ...
    echo "  sudo ${DESKTOP}/jellyfin/applyTransmission.sh"
...
Usage: sudo $0 {mount|copy|switch|nextcloud|cleanup-old|migrate|all}
  ...
  copy         COPY selfHosted → Chitragupt (rsync; originals kept)
  switch       Point .env at Chitragupt (run after verifying copy)
  nextcloud    Add Jellyfin as external folder in Nextcloud
  cleanup-old  DELETE old Desktop copies (interactive; type DELETE)
  migrate      copy + switch (never deletes Desktop data)
